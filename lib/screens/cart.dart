...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _quoteIntl() async {
    ...
    try {
      final fee = await ShopApi.intlQuote(_scope, _country.text.trim(), cart.storeId!, cart.lines);
            ...
            Row(
              ...
              if (_country.text == 'السعودية') ...[
                ...
                TextFormField(
                  ...
                  validator: (v) => (_country.text == 'السعودية' &&
                          !RegExp(r'^[A-Za-z]{4}[0-9]{4}$').hasMatch((v ?? '').trim()))
                      ? 'الصيغة: 4 أحرف ثم 4 أرقام (مثال: RRRD2929)'
                      : null,
                ),
              ],
              if (_scope == 'intl') ...[
